...
void insert(struct minheap *minh, struct minheapnode *minnode)
{
    ...
    // Check if the new node belongs to the same kind as the parent
    while (i > 0 && minnode->freq == minh->array[parent]->freq)
    {
        // Check if the new node is the right child of the parent
        if (i % 2 == 1 && parent % 2 == 0)
            break;

        // Swap the new node with its parent
        minh->array[i] = minh->array[parent];
        i = parent;
        parent = (i - 1) / 2;
    }
...
struct minheapnode *buildhuffman(char item[], int freq[], int size)
{
    ...
    while (!checksize(minh))
    {
        ...
        printf("left = (%d, %c)\t", left->freq, left->item);
        right = getminnode(minh);
        printf("right = (%d, %c)\t", right->freq, right->item);
...
void showarr(struct minheap *minh)
{
    for (int i = 0; i < minh->size; i++)
        printf("(%d, %c) ", minh->array[i]->freq, minh->array[i]->item);
    printf("\n");
}
